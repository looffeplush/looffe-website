...
$(document).ready(function(){
	...
	// 点击缩略图打开 lightbox
	$(".thumb img").off("click").on("click", function(e){
	...
	// 点击背景或关闭按钮关闭 lightbox
	$lightbox.add(".lightbox .close").off("click").on("click", function(e){
		if(e.target === this || $(e.target).hasClass("close")){
			$lightbox.fadeOut();
			// 清空 src 避免内存占用或旧图闪烁
			setTimeout(function(){ $lightboxImg.attr("src", ""); }, 200);
		}
	});
